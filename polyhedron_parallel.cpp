...
class Polyhedron {
    public:
        ...
        static vector<std::array<double,3>> shift(vector<std::array<double,3>> points, std::array<double,3> displacement) {
            vector<std::array<double,3>> output(points.size());

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < points.size(); ++i) {
                output[i] = {points[i][0]+displacement[0], points[i][1]+displacement[1], points[i][2]+displacement[2]};
            }
            return output;
        }
        ...
        static vector<std::array<double,3>> scale(vector<std::array<double,3>> points, std::array<double,3> factors) {
            ...
            #pragma omp parallel for schedule(static)
            for (int i = 0; i < points.size(); ++i) {
                output[i] = {points[i][0]*factors[0], points[i][1]*factors[1], points[i][2]*factors[2]};
            }
            return output;
        }
        ...
        static vector<std::array<double,3>> rotate(vector<std::array<double,3>> points, std::array<double,3> angles) {
            ...
            // Pre-compute trigonometric values to avoid repeated calculation
            double cos0 = cos(angles[0]), sin0 = sin(angles[0]);
            double cos1 = cos(angles[1]), sin1 = sin(angles[1]); 
            double cos2 = cos(angles[2]), sin2 = sin(angles[2]);

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < points.size(); ++i) {
                const std::array<double,3>& p = points[i];
                ...
                std::array<double,3> temp2 = {
                    temp1[0]*cos1 - temp1[2]*sin1,
                    temp1[1],
                    temp1[0]*sin1 + temp1[2]*cos1
                };

                // Rotation around Z-axis
                output[i] = {
                    temp2[0]*cos2 - temp2[1]*sin2,
                    temp2[0]*sin2 + temp2[1]*cos2,
                    temp2[2]
                };
            }
            return output;
        ...
        // Parallelized point rounding operations
        static vector<std::array<double,3>> batch_round_points(const vector<std::array<double,3>>& points) {
            vector<std::array<double,3>> rounded_points(points.size());

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < points.size(); i++) {
                rounded_points[i] = round_point(points[i]);
            }
        ...
        // Parallelized remaining rounding utility functions
        static vector<std::array<double,3>> round_point_meter_batch(const vector<std::array<double,3>>& points, set<double> meters) {
            vector<std::array<double,3>> rounded_points(points.size());

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < points.size(); i++) {
                rounded_points[i] = round_point_meter(points[i], meters);
            }
        ...
        // Parallelized batch dot product calculations
        static vector<double> batch_dot_products(const vector<std::array<double,3>>& vectors1,
                const vector<std::array<double,3>>& vectors2) {
            if (vectors1.size() != vectors2.size()) {
                throw std::invalid_argument("Vector arrays must be same size");
            }

            vector<double> results(vectors1.size());

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < vectors1.size(); i++) {
                results[i] = dot(vectors1[i], vectors2[i]);
            }
        ...
        // Parallelized multiple distance calculations
        static vector<double> batch_distance_calculations(const vector<std::array<double,3>>& points1, 
                const vector<std::array<double,3>>& points2) {
            vector<double> distances(points1.size() * points2.size());

            #pragma omp parallel for schedule(static) collapse(2)
            for (int i = 0; i < points1.size(); i++) {
                for (int j = 0; j < points2.size(); j++) {
                    int idx = i * points2.size() + j;
                    distances[idx] = distance(points1[i], points2[j]);
                }
            }
        ...
        // Parallelized batch cross product calculations
        static vector<std::array<double,3>> batch_cross_products(const vector<std::array<double,3>>& vectors1, 
                const vector<std::array<double,3>>& vectors2) {
            if (vectors1.size() != vectors2.size()) {
                throw std::invalid_argument("Vector arrays must be same size");
            }

            vector<std::array<double,3>> results(vectors1.size());

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < vectors1.size(); i++) {
                results[i] = cross3D(vectors1[i], vectors2[i]);
            }
        ...
        // Parallelized utility functions for geometric calculations
        static bool colinear(vector<std::array<double,3>> points) {
            ...
            // Parallelize point checking against line
            vector<bool> is_colinear(points.size(), true);

            #pragma omp parallel for schedule(static)
            for (int p_idx = 0; p_idx < points.size(); p_idx++) {
                const std::array<double,3>& p = points[p_idx];
                if (p == points[index]) {
                    continue;
                }
                ...
                if (Polyhedron::distance(p, point_prime) > 0.000001) {
                    is_colinear[p_idx] = false;
                }
            }

            // Check if all points are colinear
            for (bool result : is_colinear) {
                if (!result) {
                    return false;
                }
            }
        ...
        static bool coplanar(vector<std::array<double,3>> points) {
            ...
            // Parallelize coplanarity checking for remaining points
            vector<bool> is_coplanar(points.size(), true);
        ...
        // Enhanced inside_triangle with optimized matrix operations
        ...
        // Parallelized batch triangle checking function
        static vector<bool> batch_inside_triangle(const vector<std::array<std::array<double,3>,3>>& triangles, 
                const vector<std::array<double,3>>& points) {
            vector<bool> results(triangles.size() * points.size());

            #pragma omp parallel for schedule(static) collapse(2)
            for (int t_idx = 0; t_idx < triangles.size(); t_idx++) {
                for (int p_idx = 0; p_idx < points.size(); p_idx++) {
                    int result_idx = t_idx * points.size() + p_idx;
                    results[result_idx] = inside_triangle(triangles[t_idx], points[p_idx]);
                }
            }
        ...
        static vector<bool> convex_angles(vector<std::array<double,3>> circuit) {
            ...
            vector<double> cross_product_dot_normal(circuit.size());

            // Parallelize cross product calculations
            #pragma omp parallel for schedule(static)
            for (int i = 0; i < circuit.size(); i++) {
                std::array<double,3> cross_prod = Polyhedron::cross_product_triplet(
                        circuit[(i-1+circuit.size())%circuit.size()],
                        circuit[i],
                        circuit[(i+1)%circuit.size()]
                        );
                cross_product_dot_normal[i] = Polyhedron::dot(cross_prod, normal);
            }

            vector<double> signs(circuit.size());

            // Parallelize sign calculations
            #pragma omp parallel for schedule(static)
            for (int i = 0; i < cross_product_dot_normal.size(); i++) {
                signs[i] = copysign(1, cross_product_dot_normal[i]);
            }

            vector<bool> output(circuit.size());

            // Parallelize convexity checks
            #pragma omp parallel for schedule(static)
            for (int i = 0; i < signs.size(); i++) {
                output[i] = (signs[i] == signs[(i-1+signs.size())%signs.size()] || 
                        signs[i] == signs[(i+1)%signs.size()]);
            }

            return output;
        }
        static vector<bool> clockwise_angles(vector<std::array<double,3>> planar) {
            vector<bool> output(planar.size());

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < planar.size(); i++) {
                // Create local copy for this thread
                vector<std::array<double,3>> local_planar = planar;

                // Transform to local coordinate system
                vector<std::array<double,3>> temp;
                for (const std::array<double,3>& y : local_planar) {
                    temp.push_back({y[0]-local_planar[i][0], y[1]-local_planar[i][1], y[2]-local_planar[i][2]});
                }
                local_planar = temp;
                ...
                std::array<double,3> angle = {
                    0, 0, 
                    -atan2(local_planar[i][1] - local_planar[(i-1+local_planar.size())%local_planar.size()][1],
                            local_planar[i][0] - local_planar[(i-1+local_planar.size())%local_planar.size()][0])
                };

                // Rotate the points
                local_planar = Polyhedron::rotate(local_planar, angle);

                // Calculate final angle
                double theta = atan2(local_planar[(i+1)%local_planar.size()][1], 
                        local_planar[(i+1)%local_planar.size()][0]);
                output[i] = (theta < 0);
            }
        ...
        static vector<std::array<double,3>> make_planar(vector<std::array<double,3>> circuit) {
            ...
            // Parallelize the rotation and projection operations
            vector<std::array<double,3>> result(circuit.size());
            ...
            #pragma omp parallel for schedule(static)
            for (int i = 0; i < circuit.size(); i++) {
                const std::array<double,3>& p = circuit[i];
                ...
                // Project to 2D (set z=0)
                result[i] = {temp2[0], temp2[1], 0.0};
            }

            return result;
        }
        static bool is_clockwise(vector<std::array<double,3>> planar) {
            vector<double> theta_values(planar.size());
            ...
            for (int i = 0; i < planar.size(); i++) {
                ...
                // Translate to origin at current point
                std::array<double,3> x = local_planar[i];
                for (int j = 0; j < local_planar.size(); j++) {
                    local_planar[j][0] -= x[0];
                    local_planar[j][1] -= x[1];
                    local_planar[j][2] -= x[2];
                }
                ...
                theta_values[i] = atan2(local_planar[(i+1)%local_planar.size()][1], 
                        local_planar[(i+1)%local_planar.size()][0]);
            }

            // Sum all theta values
            double summa = 0;
        ...
        // Parallelized make_clockwise for multiple circuits
        static set<vector<std::array<double,3>>> make_clockwise(set<vector<std::array<double,3>>> circuits) {
            vector<vector<std::array<double,3>>> circuits_vec(circuits.begin(), circuits.end());
            vector<bool> should_reverse(circuits_vec.size());

            #pragma omp parallel for schedule(dynamic)
            for (int i = 0; i < circuits_vec.size(); i++) {
                vector<std::array<double,3>> planar = make_planar(circuits_vec[i]);
                should_reverse[i] = !is_clockwise(planar);
            }

            set<vector<std::array<double,3>>> output;
            for (int i = 0; i < circuits_vec.size(); i++) {
                vector<std::array<double,3>> circuit = circuits_vec[i];
                if (should_reverse[i]) {
        ...
        static EarClip clip_ear(vector<std::array<double,3>> circuit) {
            ...
            // Parallelize ear validation for all convex vertices
            vector<int> valid_ears;
            vector<bool> ear_validity(circuit.size(), false);
                ...
                if (is_convex[i]) {
                    ...
                    for (int j = 0; j < circuit.size(); j++) {
                        if (j != i && j != (i+1)%circuit.size() && j != (i-1+circuit.size())%circuit.size()) {
                            if (inside_triangle(triangle, circuit[j]) && 
                                    round_point(circuit[j]) != round_point(circuit[(i-1+circuit.size())%circuit.size()]) && 
                                    round_point(circuit[j]) != round_point(circuit[i]) && 
                                    round_point(circuit[j]) != round_point(circuit[(i+1)%circuit.size()])) {
                                no_break = false;
                                break;
                            }
                        }
            ...
            // Find first valid ear (sequential to maintain deterministic behavior)
            for (int i = 0; i < circuit.size(); i++) {
                if (ear_validity[i]) {
                    EarClip ear_clip;
                    ear_clip.ear = {
        ...
        static vector<std::array<std::array<double,3>,3>> triangulate(vector<std::array<double,3>> circuit) {
            ...
            while (remainder.size() > 3) {
                ...
                // Remove consecutive duplicate points (parallelizable check)
                if (remainder.size() > 1) {
                    vector<bool> to_remove(remainder.size(), false);

                    #pragma omp parallel for schedule(static)
                    for (int i = 1; i < remainder.size() + 1; i++) {
                        int curr_idx = i % remainder.size();
                        int prev_idx = i - 1;
                        if (remainder[curr_idx] == remainder[prev_idx]) {
                            to_remove[curr_idx] = true;
                        }
                    }

                    // Sequential removal to maintain indices
        ...
        static vector<std::array<double,3>>* find_exterior_circuit(set<vector<std::array<double,3>>> circuits) {
            ...
            vector<bool> is_exterior(circuits_list.size(), true);
            ...
            for (int i = 0; i < circuits_list.size(); i++) {
                ...
                for (int j = 0; j < circuits_list.size(); j++) {
                    if (j != i && is_exterior[i]) {  // Early exit if already marked non-exterior
                        for (const std::array<double,3>& point : circuits_list[j]) {
                            bool any = false;
                            for (const std::array<std::array<double,3>,3>& triangle : triangulation) {
            ...
            // Find the first exterior circuit (sequential to maintain deterministic behavior)
            for (int i = 0; i < circuits_list.size(); i++) {
                if (is_exterior[i]) {
        ...
        static vector<Polyhedron::CircuitIntersection> circuit_intersect(std::array<std::array<double,3>,2> segment, set<vector<std::array<double,3>>> circuits) {
            ...
            // Convert circuits to vector for indexed parallel processing
            vector<vector<std::array<double,3>>> circuits_vec(circuits.begin(), circuits.end());
            vector<vector<Polyhedron::CircuitIntersection>> thread_results(circuits_vec.size());

            #pragma omp parallel for schedule(dynamic)
            for (int circuit_idx = 0; circuit_idx < circuits_vec.size(); circuit_idx++) {
                const vector<std::array<double,3>>& circuit = circuits_vec[circuit_idx];
                ...
                for (int i = 0; i < circuit.size(); i++) {
                    ...
                    if (x(0) >= 0 && x(0) <= 1 && x(1) >= 0 && x(1) <= 1) {
                        ...
                        thread_results[circuit_idx].push_back(circuit_intersection);
                    }
                }
            }
            ...
            for (const auto& thread_result : thread_results) {
                output.insert(output.end(), thread_result.begin(), thread_result.end());
            }
        ...
        vector<FaceIntersection> face_intersect(std::array<std::array<double,3>,2> segment) {
            vector<vector<FaceIntersection>> thread_results(faces.size());
            ...
            for (int face_index = 0; face_index < faces.size(); face_index++) {
                ...
                for (const std::array<std::array<double,3>,3>& triangle : triangulate(*circuit)) {
                    ...
                    if (triangle_intersect != NULL) {
                        ...
                        thread_results[face_index].push_back(intersect);
        ...
        vector<int> in_faces(std::array<double,3> point) {
            vector<vector<int>> thread_results(faces.size());
            ...
            for (int face_index = 0; face_index < faces.size(); face_index++) {
                ...
                for (const std::array<std::array<double,3>,3>& triangle : Polyhedron::triangulate(*circuit)) {
                    if (Polyhedron::inside_triangle(triangle, point)) {
                        thread_results[face_index].push_back(face_index);
                        break;
                    }
                }
                delete circuit;
            ...
            vector<int> output;
            for (int face_index = 0; face_index < faces.size(); face_index++) {
                if (!thread_results[face_index].empty()) {
        ...
        bool is_inside(std::array<double,3> point) {
            // Quick check if point is on any face
            ...
            // Parallelize face intersection calculations
            vector<vector<IsInsideIntersection>> thread_intersections(faces.size());
                    ...
                    if (x(0) >= 0 && x(1) >= 0 && x(2) >= 0 && x(3) > 0) {
                        ...
                        if (!error) {
                            ...
                            if (Polyhedron::inside_triangle(triangle, p)) {
                                thread_intersections[face_index].push_back(intersection);
                                break;  // Only need one intersection per face
                            }
                        }
                    }
                }
            ...
            for (const auto& thread_result : thread_intersections) {
                output.insert(output.end(), thread_result.begin(), thread_result.end());
            }
        ...
        void round_verts(RoundPointMeter rounder) {
            // Parallelize initial vertex rounding
            #pragma omp parallel for schedule(static)
            for (int i = 0; i < verts.size(); i++) {
                verts[i] = rounder(verts[i]);
            }

            // Vertex merging loop - must remain sequential due to vector modifications
            while (true) {
                bool double_break = false;
                for (int i = 0; i < verts.size(); i++) {
            ...
            // Edge deduplication - parallelize the comparison phase
            while (true) {
                vector<pair<int, int>> edges_to_merge;

                // Parallelize edge comparison to find duplicates
                #pragma omp parallel
                {
                    vector<pair<int, int>> local_merges;
                    ...
                    #pragma omp critical
                    {
                        edges_to_merge.insert(edges_to_merge.end(), local_merges.begin(), local_merges.end());
                    }
                }

                // Sequential processing of merges (due to index dependencies)
                if (edges_to_merge.empty()) break;

                // Process only the first merge to maintain consistency
                pair<int, int> merge = edges_to_merge[0];
                int i = merge.first;
                int j = merge.second;
            ...
            // Face deduplication - parallelize the comparison phase
            while (true) {
                vector<pair<int, int>> faces_to_merge;

                // Parallelize face comparison to find duplicates
                #pragma omp parallel
                {
                    vector<pair<int, int>> local_merges;
                    ...
                    #pragma omp critical
                    {
                        faces_to_merge.insert(faces_to_merge.end(), local_merges.begin(), local_merges.end());
                    }
                }

                // Sequential processing of face merges
                if (faces_to_merge.empty()) break;

                // Process only the first merge to maintain consistency
                pair<int, int> merge = faces_to_merge[0];
                int i = merge.first;
                int j = merge.second;
        ...
        string exp() {
            ...
            // Generate vertex data (parallelizable)
            vector<string> vertex_lines(verts.size());

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < verts.size(); i++) {
                const std::array<double,3>& vert = verts[i];
                vertex_lines[i] = to_string(vert[0]) + " " + to_string(vert[2]) + " " + to_string(vert[1]) + "\n";
            }

            // Append vertex lines
            for (const string& line : vertex_lines) {
                output2 += line;
            }

            // Process faces in parallel to generate face data
            vector<string> face_lines;
            vector<vector<string>> thread_face_lines(faces.size());
            vector<int> thread_face_counts(faces.size(), 0);
            ...
            for (int face_index = 0; face_index < faces.size(); face_index++) {
                ...
                if (circs.size() == 1) {
                    thread_face_counts[face_index] = 1;
                    string face_line = to_string(circuit->size()) + " ";
                    for (const std::array<double,3>& point : *circuit) {
                        int index = std::distance(verts.begin(), find(verts.begin(), verts.end(), point));
                        face_line += to_string(index) + " ";
                    }
                    face_line += "\n";
                    thread_face_lines[face_index].push_back(face_line);
                } else {
                    vector<std::array<std::array<double,3>,3>> triangulation = Polyhedron::triangulate(*circuit);
                    thread_face_counts[face_index] = triangulation.size();

                    for (const std::array<std::array<double,3>,3>& triangle : triangulation) {
                        string triangle_line = "3 ";
                        for (const std::array<double,3>& point : triangle) {
                            int index = std::distance(verts.begin(), find(verts.begin(), verts.end(), point));
                            triangle_line += to_string(index) + " ";
                        }
                        triangle_line += "\n";
                        thread_face_lines[face_index].push_back(triangle_line);
                    }
                }
                delete circuit;
            }

            // Collect face data and count
            int face_count = 0;
            for (int i = 0; i < faces.size(); i++) {
                face_count += thread_face_counts[i];
                for (const string& line : thread_face_lines[i]) {
                    face_lines.push_back(line);
                }
            }

            // Append face lines
            for (const string& line : face_lines) {
                output2 += line;
            }
        ...
        string dump() {
            ...
            // Parallelize vertex string generation
            vector<string> vertex_lines(verts.size());

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < verts.size(); i++) {
                const std::array<double,3>& vert = verts[i];
                vertex_lines[i] = to_string(vert[0]) + " " + to_string(vert[1]) + " " + to_string(vert[2]) + "\n";
            }
            ...
            // Parallelize edge string generation
            vector<string> edge_lines(edges.size());

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < edges.size(); i++) {
                const set<int>& edge = edges[i];
                string edge_line = "2 ";
                for (const int& index : edge) {
                    edge_line += to_string(index) + " ";
                }
                edge_line += "\n";
                edge_lines[i] = edge_line;
            }

            for (const string& line : edge_lines) {
                output += line;
            }

            // Parallelize face string generation
            vector<string> face_lines(faces.size());

            #pragma omp parallel for schedule(static)
            for (int i = 0; i < faces.size(); i++) {
                const set<int>& face = faces[i];
                string face_line = to_string(face.size()) + " ";
                for (const int& index : face) {
                    face_line += to_string(index) + " ";
                }
                face_line += "\n";
                face_lines[i] = face_line;
            }
        ...
        void loads(string text){
            ...
            // First pass: split text into lines (parallelizable)
            vector<string> lines;
            string temp_text = text;
            while (temp_text.size()) {
                std::string line;
                if (temp_text.find("\n") != std::string::npos) {
                    line = temp_text.substr(0,temp_text.find("\n")+1);
                    temp_text = temp_text.substr(temp_text.find("\n")+1,temp_text.size()-1-1);
                } else {
                    line = temp_text;
                    temp_text = "";
                }
            ...
            // Parallel line processing for tokenization
            vector<vector<string>> split_lines(lines.size());
            ...
            for (int line_idx = 0; line_idx < lines.size(); line_idx++) {
                ...
                while (temp_line.size()) {
                    string token;
                    if (temp_line.find(" ") != std::string::npos) {
                        token = temp_line.substr(0, temp_line.find(" "));
                        temp_line = temp_line.substr(temp_line.find(" ")+1,temp_line.size()-temp_line.find(" ")-1);
                    } else {
                        token = temp_line;
                        temp_line = "";
                    }
                ...
                split_lines[line_idx] = tokens;
            }

            // Sequential processing of parsed lines (due to state dependencies)
            for (int line_idx = 0; line_idx < split_lines.size(); line_idx++) {
                const vector<string>& split_line = split_lines[line_idx];
        ...
        // Parallelized circuit overlap checking
        static bool circuit_overlap(set<vector<std::array<double,3>>> circuits1, set<vector<std::array<double,3>>> circuits2) {
            ...
            // Parallelize intersection checking between circuit edges
            vector<bool> thread_found_intersection(circuit1->size(), false);

            #pragma omp parallel for schedule(dynamic)
            for (int i = 0; i < circuit1->size(); i++) {
                if (thread_found_intersection[i]) continue; // Early exit if already found
                ...
                for (int j = 0; j < circuit2->size(); j++) {
                    std::array<double,3> p3 = (*circuit2)[(j-1+circuit2->size())%circuit2->size()];
                    std::array<double,3> p4 = (*circuit2)[j];

                    std::array<double,3>* point = Polyhedron::intersect_segments({p1, p2}, {p3, p4});
                    if (point != NULL) {
                        thread_found_intersection[i] = true;
                        delete point;
                        break;
                    }
                }
            ...
            for (bool result : thread_found_intersection) {
                if (result) {
                    found_overlap = true;
                    break;
                }
        ...
        // Parallelized project_on_face_plane for batch operations
        vector<std::array<double,3>> project_points_on_face_plane(int face_index, const vector<std::array<double,3>>& points) {
            vector<std::array<double,3>>* circuit = circuit_cut(make_clockwise(this->circuits(face_index)));
            if (!circuit) return {};
            ...
            vector<std::array<double,3>> projected_points(points.size());

            #pragma omp parallel for schedule(static)
            for (int p_idx = 0; p_idx < points.size(); p_idx++) {
                const std::array<double,3>& point = points[p_idx];
                ...
                for (int i=0; i < 3; i++) {
                    projected_points[p_idx][i] = point[i]+x(0)*vec0[i];
                }
            }
        ...
        vector<RayIntersection> project_ray_on_face_plane_and_intersect(int face_index, std::array<std::array<double,3>,2> ray) {
            ...
            const set<int>& face_edges = this->faces[face_index];
            vector<int> edge_indices(face_edges.begin(), face_edges.end());
            vector<vector<RayIntersection>> thread_results(edge_indices.size());
            ...
            for (int i = 0; i < edge_indices.size(); i++) {
                ...
                for (int j = 0; j < 3; j++) {
                    point1[j] = x(0)*vec[j]+p1[j];
                }
                std::array<double,3> point2;
                for (int j = 0; j < 3; j++) {
                    point2[j] = x(1)*edge[0][j]+x(2)*edge[1][j];
                }
                ...
                if (Polyhedron::distance(point1, point2) <= pow(10,-5) && round_float(x(1)) >= 0 && round_float(x(1)) <= 1 && round_float(x(0)) >= 0) {
                    ...
                    thread_results[i].push_back(ray_intersection);
                }
            }
        ...
        static Polyhedron::CircuitIntersections intersect_circuits(vector<std::array<double,3>> circuit1,vector<std::array<double,3>> circuit2) {
            // Pre-compute triangulations once
            vector<std::array<std::array<double,3>,3>> triangulation1 = Polyhedron::triangulate(circuit1);
            vector<std::array<std::array<double,3>,3>> triangulation2 = Polyhedron::triangulate(circuit2);

            // Parallel intersection finding for circuit1 edges against circuit2 triangles
            vector<set<std::array<double,3>>> thread_intersections1(circuit1.size());
            ...
            for (int i = 0; i < circuit1.size(); i++) {
                ...
                for (const std::array<std::array<double,3>,3>& triangle : triangulation2) {
                    Polyhedron::TriangleIntersection* intersect = Polyhedron::intersect_triangle({p1,p2}, triangle);
                    if (intersect != NULL) {
                        thread_intersections1[i].insert(intersect->point);
                        delete intersect;
                    }
                }
            }

            // Parallel intersection finding for circuit2 edges against circuit1 triangles
            vector<set<std::array<double,3>>> thread_intersections2(circuit2.size());
            ...
            for (int i = 0; i < circuit2.size(); i++) {
                ...
                for (const std::array<std::array<double,3>,3>& triangle : triangulation1) {
                    Polyhedron::TriangleIntersection* intersect = Polyhedron::intersect_triangle({p1,p2}, triangle);
                    if (intersect != NULL) {
                        thread_intersections2[i].insert(intersect->point);
                        delete intersect;
                    }
                }
            }
            ...
            for (const auto& thread_result : thread_intersections1) {
                intersections.insert(thread_result.begin(), thread_result.end());
            }
            for (const auto& thread_result : thread_intersections2) {
                intersections.insert(thread_result.begin(), thread_result.end());
            }
            ...
            if (intersections.size() > 1) {
                ...
                // Parallelize projection calculations
                sorted_intersections.resize(intersections_vector.size());

                #pragma omp parallel for schedule(static)
                for (int idx = 0; idx < intersections_vector.size(); idx++) {
                    const std::array<double,3>& intersection = intersections_vector[idx];
                    ...
                    sorted_intersections[idx].alpha = x(0);
                    sorted_intersections[idx].point = intersection;
                }
            ...
            // Parallelize overlap validation for edge segments
            if (circuit_intersections.sorted_intersections.size() > 1) {
                vector<bool> valid_overlaps(circuit_intersections.sorted_intersections.size()-1, false);
                ...
                for (int index = 0; index < circuit_intersections.sorted_intersections.size()-1; index++) {
                    ...
                    // Check if midpoint is inside both circuits
                    for (const std::array<std::array<double,3>,3>& triangle : triangulation1) {
                        if (Polyhedron::inside_triangle(triangle, midpoint)) {
                            any1 = true;
                            break;
                        }
                    }
                    ...
                    if (any1 && any2) {
                        valid_overlaps[index] = true;
                    }
                }

                // Collect valid edges
                for (int index = 0; index < valid_overlaps.size(); index++) {
                    if (valid_overlaps[index]) {
                        std::array<double,3> p1 = circuit_intersections.sorted_intersections[index];
                        std::array<double,3> p2 = circuit_intersections.sorted_intersections[index+1];
        ...
        static vector<vector<std::array<double,3>>> path_along_faces(std::array<double,3> p1, std::array<double,3> p2, vector<int> face_path, Polyhedron poly, Polyhedron other_poly) {
            ...
            // Remove negative alpha intersections
            ...
            if (intersections.size()) {
                ...
                if (p != p1) {
                    // Parallelize face checking for next recursion candidates
                    vector<bool> face_valid(other_poly.faces.size(), false);
                    vector<bool> face_visited(other_poly.faces.size(), false);

                    // Check which faces are visited
                    for (int face_index2 = 0; face_index2 < other_poly.faces.size(); face_index2++) {
                        for (const int& face : face_path) {
                            if (face_index2 == face) {
                                face_visited[face_index2] = true;
                                break;
                            }
                        }
                    }

                    // Parallelize face validity checking
                    #pragma omp parallel for schedule(dynamic)
                    for (int face_index2 = 0; face_index2 < other_poly.faces.size(); face_index2++) {
                        if (face_visited[face_index2]) {
                            continue;
                        }
                        ...
                        face_valid[face_index2] = any;
                        delete circuit;
                    }

                    // Sequential recursive calls (must remain sequential due to path dependencies)
                    for (int face_index2 = 0; face_index2 < other_poly.faces.size(); face_index2++) {
                        if (!face_visited[face_index2] && face_valid[face_index2]) {
        ...
        static set<vector<std::array<double,3>>> circuit_helper(
            ...
            if (start.empty()) {
                // Parallelize initial loop structure - each start edge can be processed independently
                vector<set<std::array<double,3>>> edges_vec(edges.begin(), edges.end());
                vector<set<vector<std::array<double,3>>>> thread_circuits(edges_vec.size());

                #pragma omp parallel for schedule(dynamic)
                for (int start_idx = 0; start_idx < edges_vec.size(); start_idx++) {
                    const set<std::array<double,3>>& start_edge = edges_vec[start_idx];
                    std::array<double,3> point = *start_edge.begin();

                    set<set<std::array<double,3>>> temp = edge_lookup.at(point);
                    ...
                    for (const set<std::array<double,3>>& current_edge : temp) {
                        ...
                        if (coplanar(path_set)) {
                            ...
                            thread_circuits[start_idx].insert(result.begin(), result.end());
                        }
                    }
                }

                // Collect results from all threads
                for (const auto& thread_result : thread_circuits) {
                    circuits.insert(thread_result.begin(), thread_result.end());
                }
            } else {
                // Recursive logic remains sequential due to dependencies
                if (current == start) {
                    return {path};
                }
            ...
            // Parallelize duplicate removal logic
            vector<vector<std::array<double,3>>> circuits_list(circuits.begin(), circuits.end());
            vector<bool> should_keep(circuits_list.size(), true);

            #pragma omp parallel for schedule(dynamic)
            for (int i = 0; i < circuits_list.size(); i++) {
                if (!should_keep[i]) continue;

                for (int j = i + 1; j < circuits_list.size(); j++) {
                    if (!should_keep[j]) continue;
            ...
            // Rebuild circuits set without removed elements
            circuits.clear();
            for (int i = 0; i < circuits_list.size(); i++) {
                if (should_keep[i]) {
                    circuits.insert(circuits_list[i]);
                }
            }
            ...
            for (int poly_index = 0; poly_index < 2; poly_index++) {
                for (int face_index = 0; face_index < polys[poly_index].faces.size(); face_index++) {
                    ...
                    if (cofacial_points.size() > 2) {
                        ...
                        // Parallelize the edge computation for point pairs
                        int n = cofacial_points.size();
                        int total_pairs = (n * (n - 1)) / 2;
                        vector<set<std::array<double,3>>> potential_edges(total_pairs);

                        #pragma omp parallel for schedule(dynamic)
                        for (int pair_idx = 0; pair_idx < total_pairs; pair_idx++) {
                            // Convert linear index back to (index1, index2) pair
                            int index1 = 0;
                            int remaining = pair_idx;
                            while (remaining >= (n - 1 - index1)) {
                                remaining -= (n - 1 - index1);
                                index1++;
                            }
                            int index2 = index1 + 1 + remaining;
                            ...
                            for (int poly_idx = 0; poly_idx < 2; poly_idx++) {
                                ...
                                if (should_create_edge) {
                                    potential_edges[pair_idx] = {point1, point2};
                                    break;
                                }
                            }
                        }

                        // Collect valid edges from parallel computation
            ...
            for (int face_index = 0; face_index < poly.faces.size(); face_index++) {
                ...
                while (index1 < circuit_sets.size()) {
                    ...
                    while (index2 < circuit_sets.size()) {
                        ...
                        if (find_exterior_circuit(make_clockwise(filtered_set)) != NULL) {
        ...
        Polyhedron intersect(Polyhedron other) {
            ...
            // Check if all edges of one poly are contained in the other
            ...
            for (int poly_index = 0; poly_index < polys.size(); poly_index++) {
                ...
                // Thread-safe storage for results from each vertex
                vector<set<set<std::array<double,3>>>> thread_new_edges(poly.verts.size());
                vector<set<std::array<double,3>>> thread_leaves(poly.verts.size());
                vector<map<std::array<double,3>,set<int>>> thread_face_lookups(poly.verts.size());
                vector<bool> thread_root_in_face(poly.verts.size());
                vector<bool> thread_root_in_poly(poly.verts.size());
                ...
                for (int vert_index = 0; vert_index < poly.verts.size(); vert_index++) {
                    ...
                    thread_root_in_face[vert_index] = root_in_face;

                    set<std::array<double,3>> leaves;
                    bool root_in_poly = other_poly.is_inside(vert);
                    thread_root_in_poly[vert_index] = root_in_poly;
                    ...
                    for (const set<int>& edge : poly.edges) {
                        ...
                        for (int i = 0; i < intersects.size();) {
                            ...
                            if (j < intersects.size()) {
                                thread_new_edges[vert_index].insert({intersects[i].point, intersects[j].point});
                                cout << endl;
                        ...
                        if (intersects.size()) {
                            ...
                            if (root_in_poly) {
                                thread_new_edges[vert_index].insert({poly.verts[v_i1], intersects[0].point});
                            } else {
                                for (int i = 1; i < intersects.size(); i++) {
                                    if (round_float(intersects[i].alpha) > round_float(intersects[0].alpha)) {
                                        ...
                                        thread_new_edges[vert_index].insert({intersects[0].point, intersects[i].point});
                                        break;
                                    }
                                }
                            }
                            ...
                            if (root_in_poly) {
                                thread_new_edges[vert_index].insert({poly.verts[v_i1], poly.verts[v_i2]});
                            }
                        }
                    }

                    thread_leaves[vert_index] = leaves;
                    thread_face_lookups[vert_index] = face_lookup;
                }

                // Collect new edges from parallel processing
                set<set<std::array<double,3>>> collected_new_edges;
                for (const auto& thread_edges : thread_new_edges) {
                    collected_new_edges.insert(thread_edges.begin(), thread_edges.end());
                }

                // Sequential gift-wrapping processing for each vertex with leaves
                set<set<std::array<double,3>>> seen_leaves;

                for (int vert_index = 0; vert_index < poly.verts.size(); vert_index++) {
                    std::array<double,3> vert = poly.verts[vert_index];
                    const set<std::array<double,3>>& leaves = thread_leaves[vert_index];
                    const map<std::array<double,3>,set<int>>& face_lookup = thread_face_lookups[vert_index];
                    bool root_in_poly = thread_root_in_poly[vert_index];
                    bool root_in_face = thread_root_in_face[vert_index];
                    ...
                    if (leaves.size() > 2 && seen_leaves.find(rounded_leaves) == seen_leaves.end()) {
                        ...
                        for (int index = 0; index < path.size(); index++) {
                            ...
                            set<int> starting_faces;
                            for (const auto& lookup_pair : face_lookup) {
                                if (lookup_pair.first == p1) {
                                    starting_faces = lookup_pair.second;
                                    break;
                                }
                            }
                ...
                // Collect edges from parallel processing first
                for (const auto& thread_edges : thread_new_edges) {
                    collected_new_edges.insert(thread_edges.begin(), thread_edges.end());
                }

                // Sequential gift-wrapping processing
                ...
                edge_sets_per_poly[poly_index].push_back(collected_new_edges);
            }
            return edge_sets_to_poly(edge_sets_per_poly, polys);
        }
        ...
        static Polyhedron add_subtract_helper(Polyhedron poly1, Polyhedron poly2) {
            ...
            // Parallelize face pairing detection
            vector<vector<pair<int, int>>> thread_pairings(faces1.size());
                    ...
                    if (coplanar(vector<std::array<double,3>>(all_points.begin(), all_points.end()))) {
                        for (const set<std::array<double,3>>& edge1 : face1) {
                            for (const set<std::array<double,3>>& edge2 : face2) {
                                ...
                                if (points_on_edge1 == 2) {
                                    thread_pairings[face_index1].push_back({face_index1, face_index2});
                                    double_break = true;
                                    break;
                                } else if (points_on_edge2 == 2) {
                                    thread_pairings[face_index1].push_back({face_index1, face_index2});
            ...
            // Collect pairings from all threads
            for (const auto& thread_pairing : thread_pairings) {
                pairings.insert(pairings.end(), thread_pairing.begin(), thread_pairing.end());
            }
            ...
            // Process face combinations (sequential due to complex dependencies)
            for (int poly_index = 0; poly_index < 2; poly_index++) {
            ...
            // Parallelize edge intersection processing within faces
            #pragma omp parallel for schedule(dynamic)
                    ...
                    for (auto edge1_it = new_faces[face_index].begin(); edge1_it != new_faces[face_index].end(); ++edge1_it) {
                        for (auto edge2_it = new_faces[face_index].begin(); edge2_it != new_faces[face_index].end(); ++edge2_it) {
                            ...
                            if (intersect != nullptr) {
                                ...
                                if (!intersect_on_endpoint) {
                                    // Note: This modification of shared data structure isn't thread-safe
                                    // Would need synchronization or sequential processing for correctness
            ...
            // Rest of the function remains sequential due to complex dependencies...
            // (Merging coplanar faces, final circuit processing, building final polyhedron)

            // Merge coplanar faces (sequential due to complex state dependencies)
            bool updated = true;
            while (updated) {
                updated = false;
            ...
            // Final circuit processing (sequential)
            int face_index = 0;
            ...
            // Build final polyhedron (sequential)
            Polyhedron poly;
