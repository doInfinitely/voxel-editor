...
class Polyhedron {
  public:
   ...
   struct RayIntersection {
    ...
    std::array<vector<set<set<std::array<double,3>>>>,2> intersect(Polyhedron other) {
        std::array<vector<set<set<std::array<double,3>>>>,2> output;
    ...
    for (const vector<set<set<std::array<double,3>>>>& edge_sets : output) {
        for (const set<set<std::array<double,3>>>& edge_set : edge_sets) {
            for (const set<std::array<double,3>>& edge : edge_set) {
                ...
                it++;
                std::array<double,3> p2 = Polyhedron::round_point(*it);
                if (p1 == p2 || edge.size() == 1) {
                    continue;
                }
